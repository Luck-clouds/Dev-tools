...
/** 将可扩展消息类型转换成紧凑的搜索结果摘要。 */
function preview(message) {
  const labels = {
    image: `[图片] ${message.payload.fileName || ""}`,
    video: `[视频] ${message.payload.fileName || ""}`,
    file: `[文件] ${message.payload.fileName || ""}`,
  };
  return labels[message.type] || message.payload.text || `[${message.type}]`;
}
</script>
    ...
    >
      ...
      <div class="search-input">
        <Search :size="19" /><input
          v-model="query"
          autofocus
          placeholder="搜索昵称、文字或附件文件名"
        />
      </div>
      <div class="search-result-list">
        <div v-if="!query.trim()" class="search-empty">
          输入关键词，在全部 SQLite 历史消息中查找。
        </div>
